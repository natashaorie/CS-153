...
00000030 <PScheduler>:
  ...
  90:	e8 85 03 00 00       	call   41a <setprior>
    for (i = 0; i <  3; i++) {
	pid = fork();
  95:	e8 e0 02 00 00       	call   37a <fork>
	if (pid > 0 ) {
  9a:	83 f8 00             	cmp    $0x0,%eax
  9d:	7e 47                	jle    e6 <PScheduler+0xb6>
    printf(1, "\n  Step 2: testing the priority scheduler and setpriority(int priority)) systema call:\n");
    printf(1, "\n  Step 2: Assuming that the priorities range between range between 0 to 31\n");
    printf(1, "\n  Step 2: 0 is the highest priority. All processes have a default priority of 10\n");
    printf(1, "\n  Step 2: The parent processes will switch to priority 0\n");
  ...
  a9:	7e 2f                	jle    da <PScheduler+0xaa>
		for (i = 0; i <  3; i++) {
			wait(&exit_status);
  ab:	8d 5d f4             	lea    -0xc(%ebp),%ebx
  ae:	89 1c 24             	mov    %ebx,(%esp)
  b1:	e8 d4 02 00 00       	call   38a <wait>
  b6:	89 1c 24             	mov    %ebx,(%esp)
  b9:	e8 cc 02 00 00       	call   38a <wait>
  be:	89 1c 24             	mov    %ebx,(%esp)
  c1:	e8 c4 02 00 00       	call   38a <wait>

		}
                     printf(1,"\n if processes with highest priority finished first then its correct \n");
  c6:	c7 44 24 04 b0 09 00 	movl   $0x9b0,0x4(%esp)
  cd:	00 
  ...
  e1:	e8 9c 02 00 00       	call   382 <exit>
    setprior(0);
    for (i = 0; i <  3; i++) {
	pid = fork();
	if (pid > 0 ) {
  ...
  e6:	74 1c                	je     104 <PScheduler+0xd4>
				asm("nop"); }}
		...
		exit(0);
			...
			printf(2," \n Error \n");
  e8:	c7 44 24 04 2c 0a 00 	movl   $0xa2c,0x4(%esp)
  ef:	00 
  f0:	c7 04 24 02 00 00 00 	movl   $0x2,(%esp)
  f7:	89 45 e4             	mov    %eax,-0x1c(%ebp)
  fa:	e8 e1 03 00 00       	call   4e0 <printf>
  ff:	8b 45 e4             	mov    -0x1c(%ebp),%eax
 102:	eb 9b                	jmp    9f <PScheduler+0x6f>
	pid = fork();
	if (pid > 0 ) {
		continue;}
	else if ( pid == 0) {
		...
		setprior(30-10*i);	
 104:	6b db f6             	imul   $0xfffffff6,%ebx,%ebx
 107:	83 c3 1e             	add    $0x1e,%ebx
 10a:	89 1c 24             	mov    %ebx,(%esp)
 10d:	e8 08 03 00 00       	call   41a <setprior>
 112:	ba 50 c3 00 00       	mov    $0xc350,%edx
 117:	90                   	nop
    printf(1, "\n  Step 2: testing the priority scheduler and setpriority(int priority)) systema call:\n");
    printf(1, "\n  Step 2: Assuming that the priorities range between range between 0 to 31\n");
    printf(1, "\n  Step 2: 0 is the highest priority. All processes have a default priority of 10\n");
    ...
    setprior(0);
    for (i = 0; i <  3; i++) {
 118:	b8 10 27 00 00       	mov    $0x2710,%eax
 ...
 124:	75 fa                	jne    120 <PScheduler+0xf0>
	if (pid > 0 ) {
		continue;}
	else if ( pid == 0) {
		...
		setprior(30-10*i);	
		for (j=0;j<50000;j++) {
 126:	83 ea 01             	sub    $0x1,%edx
 129:	75 ed                	jne    118 <PScheduler+0xe8>
			for(k=0;k<10000;k++) {
				asm("nop"); }}
		printf(1, "\n child# %d with priority %d has finished! \n",getpid(),30-10*i);		
 12b:	e8 da 02 00 00       	call   40a <getpid>
 130:	89 5c 24 0c          	mov    %ebx,0xc(%esp)
 134:	c7 44 24 04 80 09 00 	movl   $0x980,0x4(%esp)
 13b:	00 
 13c:	c7 04 24 01 00 00 00 	movl   $0x1,(%esp)
 143:	89 44 24 08          	mov    %eax,0x8(%esp)
 147:	e8 94 03 00 00       	call   4e0 <printf>
		exit(0);
 14c:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
 153:	e8 2a 02 00 00       	call   382 <exit>
...
putc(int fd, char c)
{
 ...
 4ba:	e8 eb fe ff ff       	call   3aa <write>
    buf[i++] = digits[x % base];
  }while((x /= base) != 0);
  if(neg)
    buf[i++] = '-';
...
{
  write(fd, &c, 1);
 526:	e8 7f fe ff ff       	call   3aa <write>
...
putc(int fd, char c)
{
 ...
 596:	e8 0f fe ff ff       	call   3aa <write>
      } else if(c == '%'){
        putc(fd, c);
      } else {
        // Unknown % sequence.  Print it to draw attention.
...
{
  write(fd, &c, 1);
 5b3:	e8 f2 fd ff ff       	call   3aa <write>
  int c, i, state;
  uint *ap;
...
putc(int fd, char c)
{
 ...
 640:	e8 65 fd ff ff       	call   3aa <write>
      } else if(c == 's'){
        s = (char*)*ap;
        ap++;
        if(s == 0)
...
putc(int fd, char c)
{
 ...
 670:	e8 35 fd ff ff       	call   3aa <write>
...
putc(int fd, char c)
{
 ...
 6c0:	e8 e5 fc ff ff       	call   3aa <write>
          putc(fd, *s);
          s++;
        }
      } else if(c == 'c'){
...
morecore(uint nu)
{
 ...
 7ca:	e8 43 fc ff ff       	call   412 <sbrk>
  if(p == (char*)-1)
